...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($checkEmailResult && $checkEmailResult->num_rows > 0) {
        echo "<script>alert('Email address already exists'); history.back();</script>";
        exit;
    }
    ...
    if ($conn->query($insertQuery) === TRUE) {
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'your_email@gmail.com'; // Replace with your email (optional or via env variable)
            $mail->Password = getenv('SMTP_PASSWORD'); // Secure: use environment variable
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
            $mail->Port = 465;

            $mail->setFrom('your_email@gmail.com', 'ES Santos Surveying Services');
